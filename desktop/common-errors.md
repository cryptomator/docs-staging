...
This page collects errors users frequently run into and their known solutions. For general diagnostic steps such as collecting log files or enabling debug mode, see [Troubleshooting](/desktop/troubleshooting/.md).

## Vault Appears Read-Only on Windows[​](#read-only-vault-windows "Direct link to Vault Appears Read-Only on Windows")

**Symptoms:** On Windows with a [WinFsp](/desktop/volume-type/.md#winfsp) volume type, an unlocked vault behaves as if it were read-only. Copying or pasting files fails with "Permission denied", and new files or folders cannot be created or modified inside the vault.
