...
[Back up the database](/hub/self-hosting-guide/operations/.md#backup) first. Image versions are pinned in `compose.yaml`; to upgrade, change the tags and run `docker compose up -d`. Hub applies its database migrations automatically at start. For Keycloak, follow the [Keycloak upgrade guide](https://www.keycloak.org/docs/latest/upgrading/).
